...
int StartMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  if(LoadROM("/sd/roms/msx/bios/CMOS.ROM",sizeof(RTC),(byte *)RTC))
  { if(Verbose) printf("CMOS.ROM.."); }
  else memcpy(RTC,RTCInit,sizeof(RTC));

  /* Try loading Kanji alphabet ROM */
  if((Kanji=LoadROM("/sd/roms/msx/bios/KANJI.ROM",0x20000,0)))
  { if(Verbose) printf("KANJI.ROM.."); }

  /* Try loading RS232 support ROM to slot */
  if((P=LoadROM("/sd/roms/msx/bios/RS232.ROM",0x4000,0)))
  {
  ...
  if(!MODEL(MSX_MSX1)&&OPTION(MSX_MSXDOS2)&&(MemMap[3][1][2]!=EmptyRAM)&&!ROMData[2])
    if(LoadCart("/sd/roms/msx/bios/MSXDOS2.ROM",2,MAP_GEN16))
  ...
  if(!MODEL(MSX_MSX1))
  {
    ...
    if((J<MAXSLOTS)&&LoadCart("/sd/roms/msx/bios/PAINTER.ROM",J,0)) ++J;
  }

  /* Load FMPAC cartridge */
  for(;(J<MAXSLOTS)&&ROMData[J];++J);
  if((J<MAXSLOTS)&&LoadCart("/sd/roms/msx/bios/FMPAC.ROM",J,MAP_FMPAC)) ++J;

  /* Load Konami GameMaster2/GameMaster cartridges */
  for(;(J<MAXSLOTS)&&ROMData[J];++J);
  if(J<MAXSLOTS)
  {
    if(LoadCart("/sd/roms/msx/bios/GMASTER2.ROM",J,MAP_GMASTER2)) ++J;
    else if(LoadCart("/sd/roms/msx/bios/GMASTER.ROM",J,0)) ++J;
  }
...
void TrashMSX(void)
{
  ...
  if(SaveCMOS)
  {
    ...
    if(!(F=fopen("/sd/roms/msx/bios/CMOS.ROM","wb"))) SaveCMOS=0;
    else
    {
...
int ResetMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  if((Mode^NewMode)&MSX_MODEL)
  {
    ...
    switch(NewMode&MSX_MODEL)
    {
      case MSX_MSX1:
        ...
        P1=LoadROM("/sd/roms/msx/bios/MSX.ROM",0x8000,0);
        PRINTRESULT(P1);
      ...
      case MSX_MSX2:
        ...
        P1=LoadROM("/sd/roms/msx/bios/MSX2.ROM",0x8000,0);
        PRINTRESULT(P1);
        if(Verbose) printf("  Opening MSX2EXT.ROM...");
        P2=LoadROM("/sd/roms/msx/bios/MSX2EXT.ROM",0x4000,0);
        PRINTRESULT(P2);
        if(!P1||!P2) 
        {
      ...
      case MSX_MSX2P:
        ...
        P1=LoadROM("/sd/roms/msx/bios/MSX2P.ROM",0x8000,0);
        PRINTRESULT(P1);
        if(Verbose) printf("  Opening MSX2PEXT.ROM...");
        P2=LoadROM("/sd/roms/msx/bios/MSX2PEXT.ROM",0x4000,0);
        PRINTRESULT(P2);
        if(!P1||!P2) 
        {
          NewMode=(NewMode&~MSX_MODEL)|(Mode&MSX_MODEL);
  ...
  if((Mode^NewMode)&MSX_PATCHBDOS)
  {
    ...
    P1=LoadROM("/sd/roms/msx/bios/DISK.ROM",0x4000,0);
    PRINTRESULT(P1);
