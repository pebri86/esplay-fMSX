...
input_gamepad_state gamepad_input_read_raw()
{
    ...
    state.values[GAMEPAD_INPUT_SELECT] = !(gpio_get_level(IO_BTN_SELECT));
    state.values[GAMEPAD_INPUT_START] = !(gpio_get_level(IO_BTN_START));

    state.values[GAMEPAD_INPUT_A] = !(gpio_get_level(IO_BTN_A));
    state.values[GAMEPAD_INPUT_B] = !(gpio_get_level(IO_BTN_B));

    state.values[GAMEPAD_INPUT_MENU] = !(gpio_get_level(IO_BTN_MENU));
...
void gamepad_init()
{
    ...
    btn_config.pin_bit_mask = (uint64_t)      //Bitmask
                              ((uint64_t)1 << IO_BTN_A) |
                              ((uint64_t)1 << IO_BTN_B) | ((uint64_t)1 << IO_BTN_SELECT);

    btn_config.pull_up_en = GPIO_PULLUP_ENABLE;      //Disable pullup
    btn_config.pull_down_en = GPIO_PULLDOWN_DISABLE; //Enable pulldown
    gpio_config(&btn_config);

    gpio_set_direction(IO_BTN_MENU, GPIO_MODE_INPUT);
    gpio_set_pull_mode(IO_BTN_MENU, GPIO_PULLUP_ONLY);

    gpio_set_direction(IO_BTN_START, GPIO_MODE_INPUT);
